...
contract CountdownSessionManager {
  ...
  uint256 private countdownResetTimes; //  number of times, when timer got to 0
  ...
  struct SessionInfo {
    uint256 purchaseNumberForOngoingSession;   //  separate purchase made during session
    uint256 sharesPurchasedForOngoingSession;  //  shares purchased during session
  ...
  mapping (uint256 => SessionInfo) private sessionInfoForIdx; //  session info after countdown reset time idx
  ...
  function sharesPurchased(uint256 _shareNumber, address _purchaser, uint256 _rewardForPreviousShares) internal {
    ...
    session.sharesPurchasedForOngoingSession.add(_shareNumber);
  ...
  function purchasesForPurchaserInSession(address _purchaser, uint256 _session) public view returns(uint256[] purchases) {
    SessionInfo storage session = sessionInfoForIdx[_session];
    purchases = session.addressPurchasedAtIdxs[_purchaser];
  }
